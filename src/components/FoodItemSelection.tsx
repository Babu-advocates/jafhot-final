...
export function FoodItemSelection({
  isOpen,
  onClose,
  onAddItem
}: FoodItemSelectionProps) {
  ...
  const fetchFoodItems = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from('food_items').select('*, food_categories(name)').eq('status', 'available').order('name');
  ...
  const fetchCategories = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('food_categories').select('id, name').order('name');
      if (error) throw error;
      setCategories(data || []);
    } catch (error) {
      toast({
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="border rounded-md max-h-[280px] overflow-y-auto">
              {loading ? <div className="p-4 text-center text-muted-foreground">Loading items...</div> : filteredItems.length === 0 ? <div className="p-4 text-center text-muted-foreground">
                  No items found
            ...
            if (item) handleItemSelect(item);
              ...
              <SelectTrigger>
                <SelectValue placeholder="Choose a food item">
                  {selectedItem && <span className="flex items-center gap-2">
                      {selectedItem.name}
              ...
              <SelectContent>
                {loading ? <SelectItem value="loading" disabled>Loading...</SelectItem> : filteredItems.map(item => <SelectItem key={item.id} value={item.id}>
          ...
          <div>
            ...
            {!isManualQuantity ? <div className="flex gap-2">
               ...
               <SelectContent side="bottom" align="start">
                ...
                <Button type="button" variant="outline" size="icon" onClick={() => setIsManualQuantity(true)} className="shrink-0">
                  <Plus className="h-4 w-4" />
                </Button>
              </div> : <Input type="number" min="1" value={quantity} onChange={e => setQuantity(Math.max(1, parseInt(e.target.value) || 1))} className="w-full" />}
          </div>

          {/* Item Details */}
          {selectedItem && <div className="bg-muted p-4 rounded-lg space-y-2">
          ...
          <div className="flex gap-4 pt-4">
            <Button onClick={handleAddItem} disabled={!selectedItem} className="flex-1 bg-primary text-primary-foreground">
              Add Item
            </Button>
